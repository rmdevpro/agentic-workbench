...
3. **Update your plan file** - to do this
    1. Enter plan mode (call EnterPlanMode) so Claude Code edits this session's active plan file at `~/.claude/plans/<slug>.md`.
    2. Update the Session Summary these details include
    ...
    3. **Update the reading list -** Reflect on what you used this session. Update the Required and Situational reading lists in the plan file with specific file paths and a one-line reason each is needed. Bias toward inclusion — the next context pays for missing a critical doc, not for over-reading.
    4. **Write resume instructions -** Add explicit, actionable resume instructions to the plan file. Be specific:
    ...
    5. **Maintain the Work Plan -** Below the required sections, the plan file holds the general plan for completing the session's work. Make sure phases, steps, and dependencies reflect current intent.
