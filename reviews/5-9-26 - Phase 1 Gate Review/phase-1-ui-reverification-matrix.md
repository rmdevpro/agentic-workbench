...
| D2 #351 | shared with `27-a12-gate-page-password-mode.png` plus parallel-request evidence | Setup: same gated container. Host-side curl: 15 parallel POST requests to `/api/gate/login` with intentional wrong password (allowed for state-driving setup; observation is via response codes which the user would also see). | 15 parallel attempts → **13 returned 429 (rate-limited)**, 2 returned 401 (consumed remaining bucket tokens before depletion). Per `server.js:171-186`: token bucket cap=10 tokens, refill=1 token/6s, IP-keyed. Once depleted, `_consumeLoginBucket()` returns false → 429. After 60s of inactivity bucket refills back to 10. D2's brute-force rate-limit verified end-to-end. |
