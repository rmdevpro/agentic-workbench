...
| D2 #351 | shared with `27-a12-gate-page-password-mode.png` plus parallel-request evidence; UI render in `29-d2-ui-rate-limit-misleading-error.png` | Setup: same gated container. Host-side curl: 15 parallel POST requests to `/api/gate/login` with intentional wrong password. Then UI: bucket depleted via 12 parallel curl bursts; in browser `browser_click` on Sign In with valid username + WRONG_TO_TRIGGER_429 password. | **Backend layer ✓**: 15 parallel attempts → 13 returned 429 (rate-limited), 2 returned 401. Per `server.js:171-186`: token bucket cap=10 tokens, refill=1 token/6s, IP-keyed. Once depleted, `_consumeLoginBucket()` returns false → 429. **UI render finding**: After bucket fully depleted (12 parallel hits → 9× 429 + 3× 401), the gate UI's Sign In click still renders "Invalid username or password" (same text as 401). `public/gate.html:248` shows the error element on any non-OK response without distinguishing 401 from 429 — the hardcoded text is "Invalid username or password" (gate.html:line setting `<div class="login-error" id="login-error">Invalid username or password</div>`). Backend rate-limit fix works; UI doesn't surface the rate-limit state distinctly to the user. **Phase 1 gap to follow up:** gate.html needs a 429-aware error path that reads response.status and renders "Too many attempts; please wait" or similar. |
...
### Genuinely-blocked / not-yet-verified

- **A3 #328** (issue picker GraphQL string concat) — tested via A4's picker flow which made a real GraphQL call to `/api/issues?repo=github.com/different-org/rebuild-test`. The call did not crash; it returned `no_account_for_path` (auth failure, not a string-concat crash). The fix's "GraphQL string concat" verify clause requires a successfully-listed-issues case which needs a project tied to a properly-credentialed GH account. The Workbench project (id 39, repo=rmdevpro/agentic-workbench) requires github_issue when adding a task, which is circular for picker testing. **Partially verified** by A4's "call doesn't crash" observation; full positive case deferred to a follow-up with a credentialed test repo.
- **A8 #333** (`/api/auth/login` not burning Claude tokens) — needs sustained Claude session monitoring with controlled re-auth events to observe token-counter delta. Token budget concern. Deferred.
- **A13 #338** (`session-resolver` async conversion with large JSONL) — needs a 5-10MB JSONL fixture. Out of remaining context budget. Deferred. Mock-pin covers the structural side.
