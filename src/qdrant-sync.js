...
const { watch, existsSync, createReadStream } = require('fs');
...
// Qdrant rejects POST bodies > 32 MiB by default.
const QDRANT_BODY_LIMIT = 24 * 1024 * 1024;

const VALIDATE_TIMEOUT_MS = 8000;

// ── Pure utility functions (no injected deps) ───────────────────────────────
...
// ── Streaming helpers (exported for E4 test coverage) ──────────────────────
...
// embedding scales with the largest section, not the whole file.
...
// fs.readFile single-allocation path that doubles peak memory.
...
function createQdrantSync({ db, safe, config, logger }) {
  ...
  // Incremental session sync: track the last turn index we embedded.
  // ALTER is idempotent (caught by try).
  ...
  // vendor endpoint move is fixed by editing the config file rather than code.
  ...
  async function getEmbeddingConfig() {
    ...
    switch (provider) {
      ...
      case 'openai': {
        ...
        let key = await _readCodexKey() || _parseSetting('codex_api_key', '') || process.env.OPENAI_API_KEY || '';
  ...
  async function embedWithConfig(cfg, texts, dims) {
    ...
    // socket-level errors (EPIPE / ECONNRESET / etc) and HTTP 5xx + 429.
  ...
  // #212: shared retry-with-backoff helper.
  ...
  async function embed(texts, dims) {
    return embedWithConfig(await getEmbeddingConfig(), texts, dims);
  }

  // #180: build a candidate provider cfg from a (key,value) override pair so we can
  // validate before persisting.
  async function buildCandidateConfig(overrideKey, overrideValue) {
    let provider;
    ...
    switch (provider) {
      ...
      case 'openai': {
        const key = overrideKey === 'codex_api_key'
          ? (overrideValue || '')
          : (await _readCodexKey() || _parseSetting('codex_api_key', '') || process.env.OPENAI_API_KEY || '');
        const c = _providerCfg('openai');
        return { url: c.url, model: c.model, key };
      }
      case 'custom': {
  ...
  async function upsertPoints(collection, points) {
    ...
    // qdrant side doesn't wedge the sync row with a single un-retried ERROR.
  ...
  async function deletePointsByFilter(collection, filter) {
    // #212: same retry rationale as upsertPoints.
    return retryTransient(async () => {
  ...
  async function syncSessionFile(filePath, collection, parser, dims) {
    ...
    // overlap window so the new chunks blend with the prior context).
    ...
    const texts = chunks.map(c => c.text);
    // Batch embeddings in groups of 10 with delay to avoid burst rate limits.
    const allEmbeddings = [];
  ...
  async function scanCollection(collection, dirs, patterns, dims) {
    ...
    const scan = async (dir, baseDir) => {
      if (!await _pathExists(dir)) return;
      const entries = await readdir(dir, { withFileTypes: true });
      for (const entry of entries) {
        // Bail mid-scan if a settings change has stopped this run.
        if (!_running) return;
  ...
  async function scanDocs() {
    ...
    for (const p of getAdditionalPaths()) {
      if (await _pathExists(p)) dirs.push(p);
    }
  ...
  async function scanClaudeSessions() {
    ...
    if (!await _pathExists(projectsDir)) return 0;
    ...
    for (const pDir of projectDirs) {
      ...
      for (const file of files) {
        ...
        try {
          ...
          // #233: provider flipped to 'none' mid-scan.
          if (err.message === 'No embedding API key configured' && getEmbeddingProvider() === 'none') {
  ...
  async function scanGeminiSessions() {
    ...
    if (!await _pathExists(geminiBase)) return 0;
    ...
    try {
      ...
      for (const pDir of projectDirs) {
        ...
        if (!await _pathExists(chatsDir)) continue;
  ...
  async function scanCodexSessions() {
    ...
    if (!await _pathExists(codexBase)) return 0;
  ...
  // Retry with backoff before giving up; if still unreachable, schedule a
  // periodic background re-attempt.
  ...
  async function start() {
    ...
    try {
      ...
      // Probe the embedding provider once before scanning.
      const probeResult = await validateProviderConfig(await getEmbeddingConfig());
      if (!probeResult.ok) {
        logger.warn('Embedding provider unavailable — vector sync disabled until configured', {
          module: 'qdrant-sync',
          provider: (await getEmbeddingConfig()).model,
  ...
  // Stop + start. Used when settings change (provider/key updates).
  async function restart() {
    return reapplyConfig({ dropCollections: false });
  }

  // Drop every Qdrant collection this module manages.
  ...
  // serialized so rapid consecutive calls don't run overlapping stop/start/scan
  // sequences and race on the shared collections.
  //
  // Coalescing: while one apply is running, additional calls are folded into
  // a single trailing apply that runs after the current one finishes.
...
module.exports = {
  createQdrantSync,
  // #443 [E4]: exported at module level so the streaming heap-budget test can
  // exercise the actual production helpers without instantiating the factory
  // (which would require a real DB connection).
  _streamHashAndChunk,
  _streamReadAsString,
};
