...
| ANSI auth output | Auth URL detection with real escape sequences | `tests/fixtures/ansi-auth-url.txt` (deferred — captured by F7 #363 OAuth-detector extraction) |
| Chunked WS frames | Auth URL fragmented across frames | `tests/fixtures/chunked-auth-frames.bin` (deferred — same capture as F7 #363) |
| File tree fixture | Pre-created directory tree for drag-drop and file browser | Created in test workspace volume |
| Test data factory | JSONL/settings/task/session shapes for unit tests | `tests/fixtures/test-data.js` (programmatic — see `makeAssistantEntry`, `makeUserEntry`, etc. for inline construction) |
| Trigger uncaught | Crash trigger for SRV-05 | `tests/fixtures/trigger-uncaught.js` |
...
tests/
  browser/
    ...
    fixtures/
      ...
      test-data.js                 # JSONL/programmatic factories used inline
      trigger-uncaught.js          # SRV-05 crash trigger
      # ansi-auth-url.txt          — deferred to F7 #363 (OAuth detector)
      # chunked-auth-frames.bin    — deferred to F7 #363 (same capture)
