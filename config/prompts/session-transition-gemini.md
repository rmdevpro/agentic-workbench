...
3. **Update your plan file** - the plan file lives at `~/.gemini/plans/{{SESSION_ID}}.md`. Use Write or Edit to maintain it (create the file if it does not yet exist). The plan must contain these sections:
    1. **Session Summary**
...
When the checklist is complete, run `/compress`. After compression, run `/session:resume` to restore context.
